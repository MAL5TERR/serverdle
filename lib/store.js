...
// All results for one date (YYYY-MM-DD), sorted so wins come first, fewest attempts first.
function getResultsForDate(date){
  const data = loadData();
  return data.results
    .filter(r => r.date === date)
    .sort((a, b) => (b.won - a.won) || (a.attemptsUsed - b.attemptsUsed));
}
...
module.exports = { loadData, saveData, upsertResult, getLeaderboard, getUserStats, getStreak, getResultsForDate, resetUser };